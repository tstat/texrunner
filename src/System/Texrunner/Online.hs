...
runOnlineTex' command args preamble process = do
  putStrLn "starting onlinetex"
  withSystemTempDirectory "onlinetex." $ \path -> do
    ...
    case logPath of
      Nothing -> putStrLn "no logfile found"
      Just lp ->
        doesFileExist lp >>= \case
          True -> copyFile lp "/tmp/errs"
          False -> putStrLn "no logfile exists"
...
runInteractiveProcess' cmd args wd env = do
  ...
  sOut <-
    Streams.handleToInputStream hout
      >>= Streams.atEndOfInput (hClose hout)
      >>= Streams.lockingInputStream
      >>= debugStream
  sErr <-
    Streams.handleToInputStream herr
